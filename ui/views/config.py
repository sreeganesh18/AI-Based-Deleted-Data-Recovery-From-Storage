...
def get_checkpoints():
    """Returns a list of .pth files in models/checkpoints/."""
    checkpoint_dir = "models/checkpoints"
    if not os.path.exists(checkpoint_dir):
        return []
    return [os.path.basename(f) for f in glob.glob(os.path.join(checkpoint_dir, "*.pth"))]
...
with st.form("config_form"):
    ...
    disk_image_path = st.text_input(
        "Disk Image Path", 
        value=st.session_state.disk_image_path,
    ...
    if not checkpoints:
        ...
        clf_options = checkpoints
        ae_options = checkpoints
    ...
    with col1:
        selected_clf = st.selectbox(
            ...
            options=clf_options,
            index=clf_options.index(st.session_state.clf_checkpoint) if st.session_state.clf_checkpoint in clf_options else 0,
    ...
    with col2:
        selected_ae = st.selectbox(
            ...
            options=ae_options,
            index=ae_options.index(st.session_state.ae_checkpoint) if st.session_state.ae_checkpoint in ae_options else 0,
...
if submit_button:
    ...
    if not valid:
        ...
        st.session_state.clf_checkpoint = selected_clf if selected_clf != "None found" else None
        st.session_state.ae_checkpoint = selected_ae if selected_ae != "None found" else None
...
# Data Preview (outside the form to update on path change or after init)
    ...
    if valid:
        try:
            with open(st.session_state.disk_image_path, "rb") as f:
                # Read the first 512 bytes (one sector) for preview
